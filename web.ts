...
(async () => {
    try {   process.argv.push('/home/nour/Bureau/telegram/telegram-example.eim')
    ...
    imageClassifier.on('result', async (result, timeMs, imgAsJpg) => {
        ...
        if (r.bounding_boxes) {
            ...
            r.bounding_boxes = bb;
                  ...
                  if (bb.find(x => x.label === 'capsule_wire_pass')) {
                console.log('capsule_wire_pass!');
                  lastSeenmodel = Date.now();

}
else if (bb.find(x => x.label === 'capsule_wire_fail')) {
            console.log('capsule_wire_fail!');
              lastSeenmodel = Date.now();}
            
          else if (bb.find(x => x.label === 'XLR_wire_pass')) {
            console.log('XLR_wire_pass!');
              lastSeenmodel = Date.now();}
            
            
              
             if ((!bb.find(x=> x.label ==='') && Date.now() - lastSeenmodel > 30000  && !isMessageDisplayed) ){
console.log('vérifié que origin pcb à sa position exacte');
 isMessageDisplayed = true; // Mettre la variable à true pour indiquer que le message a été affiché
   lastSeenmodel = Date.now();
